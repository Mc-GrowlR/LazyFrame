...
--- 1. 已做修改添加Getter和Setter机制
--- 2. 优化代码，使用local变量缓存查询结果。防止额外查询消耗性能。
--- 3. 将Getter和Setter也加入到继承机制中。
local mt = {
    clsName = "mt",
    __get__ = {},
    __set__ = {},
...
local function NewClass(clsName, base)
    ...
    cls.new = function(...)
        ...
        --- 控件初始化函数，常用的控件都在这里进行利用原生api进行创建
...
---自定义打印函数，对原生dbg函数进行包装，添加代码位置等打印信息。
---@param ... unknown
function dbginfo(...)
    local args = { ... }
    local message_t = {}
...
---迭代xml表节点，迭代 深度为1
---更方便的xml表迭代函数，可用于xml表的批处理，用于for循环中
---@param xml Xml
---@return function
